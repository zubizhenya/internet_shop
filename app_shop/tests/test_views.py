...
class TestProductView(TestCase):
    ...
    def test_product_page_context(self):
        ...
        self.assertEqual(
            context["review_form"].__class__.__name__, ReviewForm().__class__.__name__
        )
        self.assertEqual(
            context["form"].__class__.__name__, CartAddProductForm().__class__.__name__
        )
        self.assertEqual(
            context["reviews"].first().__class__.__name__, Review().__class__.__name__
        )

    def test_product_page_post(self):
        self.client.login(email="test@test.ru", password="12345")
        response = self.client.post(
            reverse("product_detail", args=[1, "test_product"]), {"text": "test_review"}
        )
