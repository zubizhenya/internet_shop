...
class TestOrderView(TestCase):
    ...
    def test_order_page_contain_right_context(self):
        ...
        self.assertEqual(
            form_reg.__class__.__name__, UserCreateForm().__class__.__name__
        )
    ...
    def test_order_page_post(self):
        ...
        self.assertEqual(
            str(messages[1]), "Ждём подтверждения оплаты от платёжной системы."
        )
...
class TestHistoryView(TestCase):
    ...
    def setUpTestData(cls):
        ...
        order = Order.objects.create(
            user=user, address="test_address", city="test_city", card_number=888888888
        )
        order.save()
        order_item = OrderItem.objects.create(
            order=order, product=product, price=10, quantity=10
        )
        order_item.save()
...
class OrderPaymentView(TestCase):
    ...
    def test_pay_page_post(self):
        self.client.login(email="test@test.ru", password="12345")
        response = self.client.post(
            reverse("pay", args=[1]), {"card_number": "88888888", "payment_type": "1"}
        )
        self.assertEqual(response.status_code, 302)
        messages = list(get_messages(response.wsgi_request))
        self.assertEqual(len(messages), 1)
        self.assertEqual(
            str(messages[0]), "Ждём подтверждения оплаты от платёжной системы."
        )
